...
function getAllPost() {
    $.getJSON("userPost", function (response) {
        var abs = response;
        $(".loa").empty();
        $("#picload").empty();
        for (var i in abs) {
        ...
        for (var i in abs) {
            ...
            var row = "<div class='row' id=''>\n\
                         <div class='col-lg-4'>\n\
                             <div class='panel'>\n\
                                 <div class='boximg'>\n\
                                     <img src='resources/wallpics/" + abs[i].image + "' class='img-responsive'/>\n\
                                     <span class='label label-danger date'>My - Ratings</span>\n\
                         ...
                         <div class='col-lg-4'>\n\
                             <div class='panel'>\n\
                                 <div class='boximg'>\n\
                                     <img src='resources/wallpics/" + abs[parseInt(i) + 1].image + "' class='img-responsive'/>\n\
                                     <span class='label label-danger date'>My - Ratings</span>\n\
                                     <span class='likebut glyphicon glyphicon-tag'></span>\n\
                                 </div>\n\
                             </div>\n\
                          ...
                          <div class='col-lg-4'>\n\
                             <div class='panel'>\n\
                                 <div class='boximg'>\n\
                                     <img src='resources/wallpics/" + abs[parseInt(i) + 2].image + "' class='img-responsive'/>\n\
                                     <span class='label label-danger date'>My - Ratings</span>\n\
                                     <span class='likebut glyphicon glyphicon-tag'></span>\n\
                                 </div>\n\
                             </div>\n\
...
$("#cv_save").click(function () {
    $("#res-dis").empty();
    $("#pan").fadeIn(1000);
    var form = new FormData($("#cvupForm")[0]);
    $.ajax({
        url: "cvadd",
        method: "POST",
        dataType: 'json',
